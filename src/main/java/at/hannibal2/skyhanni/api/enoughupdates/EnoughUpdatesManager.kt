...
import at.hannibal2.skyhanni.api.enoughupdates.datatypes.Recipe
...
// TODO need lowest bin json at some point
...
object EnoughUpdatesManager {
    ...
    private fun parseItem(internalName: String, json: JsonObject): JsonObject? {
        ...
        json["recipe"]?.asJsonObject?.let { recipe ->
            Recipe.loadRecipe(recipe.asJsonObject, json)
        }
    ...
    fun jsonToStack(json: JsonObject?, useCache: Boolean = true, useReplacements: Boolean = false): ItemStack {
        ...
        if (json == null) return ItemStack(Items.painting)
        ...
        if (stack.item == null) return ItemStack(Item.getItemFromBlock(Blocks.stone), 0, 255)
        ...
        json["lore"]?.asJsonArray?.let { lore ->
            val displayTag = if (stack.tagCompound != null && stack.tagCompound.hasKey("display")) {
                stack.tagCompound.getCompoundTag("display")
            } else {
                NBTTagCompound()
            }
            displayTag.setTag("Lore", processLore(lore, replacements))
            val tag = if (stack.tagCompound != null) stack.tagCompound else NBTTagCompound()
    ...
    fun getPetLoreReplacements(tag: NBTTagCompound?, level: Int): Map<String, String> {
        tag ?: return emptyMap()
