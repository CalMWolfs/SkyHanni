...
data class PrimitiveRecipe(
    ...
    companion object {
            ...
            when (type) {
                ...
                "trade" -> {
                    ...
                    if (recipeJson.has("max")) {
                        ...
                        val recipe = PrimitiveRecipe(setOf(PrimitiveIngredient(recipeJson["cost"].asString, average)), output, RecipeType.TRADE)
                        EnoughUpdatesManager.registerRecipe(recipe)
                    } else {
                        val recipe = PrimitiveRecipe(setOf(PrimitiveIngredient(recipeJson["cost"].asString)), output, RecipeType.TRADE)
                        EnoughUpdatesManager.registerRecipe(recipe)
                    }
                }
                ...
                "drops" -> {
                    ...
                    val recipe = PrimitiveRecipe(ingredient, outputs, RecipeType.MOB_DROP)
                    EnoughUpdatesManager.registerRecipe(recipe)
                }
                ...
                "katgrade" -> {
                    ...
                    val recipe = PrimitiveRecipe(ingredients, output, RecipeType.KAT_UPGRADE)
                    EnoughUpdatesManager.registerRecipe(recipe)
                }
        ...
        private fun submitRecipe(
            ...
            val outputItem = PrimitiveIngredient(outputInternalName, craftAmount)
